...
class AggregatedKarhunenLoeveResults(object):
    ...
    def __init__(self, composedKLResultsAndDistributions, verbose=0):
        ...
        """
        self.__KLResultsAndDistributions__ = atLeastList(
            composedKLResultsAndDistributions
        )  # KLRL : Karhunen Loeve Result List
        ...
        self.__means__ = [0.0] * self.__field_distribution_count__
        ...
        for i in range(self.__field_distribution_count__):
            ...
            if isinstance(
                self.__KLResultsAndDistributions__[i], ot.KarhunenLoeveResult
            ):
                # initializing lifting and projecting objects.
                self.__KL_lifting__.append(
                    ot.KarhunenLoeveLifting(self.__KLResultsAndDistributions__[i])
                )
                self.__KL_projecting__.append(
                    ot.KarhunenLoeveProjection(self.__KLResultsAndDistributions__[i])
                )
                self.__isProcess__[i] = True

            # If element is a distribution
            elif isinstance(
                self.__KLResultsAndDistributions__[i],
                (ot.Distribution, ot.DistributionImplementation),
            ):
                self.__has_distributions__ = True
                if self.__KLResultsAndDistributions__[i].getMean()[0] != 0:
                    if i < 20 and self.__verbose__ > 0:
                        print(
                            "The mean value of distribution {} at index {} of type {} is not 0.".format(
                                str(
                                    '"'
                                    + self.__KLResultsAndDistributions__[i].getName()
                                    + '"'
                                ),
                                str(i),
                                self.__KLResultsAndDistributions__[i].getClassName(),
                            )
                        )
                    name_distr = self.__KLResultsAndDistributions__[i].getName()
                    self.__means__[i] = self.__KLResultsAndDistributions__[i].getMean()[
                        0
                    ]
                ...
                self.__KL_lifting__.append(
                    self.__KLResultsAndDistributions__[
                        i
        ...
        if not self.__has_distributions__:
            self.__unified_mesh__ = all_same(
                [
                    self.__KLResultsAndDistributions__[i].getMesh()
                    for i in range(self.__field_distribution_count__)
                ]
            )
            self.__unified_dimension__ = all_same(
                [
                    self.__KLResultsAndDistributions__[i]
                    .getCovarianceModel()
                    .getOutputDimension()
        ...
        if self.__field_distribution_count__ == 1:
            if hasattr(
                self.__KLResultsAndDistributions__[0], "getCovarianceModel"
            ) and hasattr(self.__KLResultsAndDistributions__[0], "getMesh"):
                # Cause when aggregated there is usage of multvariate covariance functions
                self.__isAggregated__ = (
                    self.__KLResultsAndDistributions__[0]
                    .getCovarianceModel()
                    .getOutputDimension()
                    > self.__KLResultsAndDistributions__[0].getMesh().getDimension()
                )
                print("Process seems to be aggregated. ")
            else:
                print(
                    "There is no point in passing only one process that is not aggregated"
                )
                raise TypeError

        self.threshold = max(
            [
                self.__KLResultsAndDistributions__[i].getThreshold()
                if hasattr(self.__KLResultsAndDistributions__[i], "getThreshold")
                else 1e-3
                for i in range(self.__field_distribution_count__)
            ]
        )
        # Now we gonna get the data we will usually need
        self.__process_distribution_description__ = [
            self.__KLResultsAndDistributions__[i].getName()
            for i in range(self.__field_distribution_count__)
        ]
        self._checkSubNames()
        self.__mode_count__ = [
            self.__KLResultsAndDistributions__[i].getEigenvalues().getSize()
            if hasattr(self.__KLResultsAndDistributions__[i], "getEigenvalues")
            else 1
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def __repr__(self):
        ...
        reprStr = "| ".join(
            [
                "class = ComposedKarhunenLoeveResultsAndDistributions",
                ...
                *[
                    "Covariance Model {} = ".format(str(i))
                    + covarianceList[i].__repr__()
                    for i in range(self.__field_distribution_count__)
                ],
                *[
                    "Eigen Value {} = ".format(str(i)) + eigValList[i].__repr__()
                    for i in range(self.__field_distribution_count__)
                ],
                *[
                    "Mesh {} = ".format(str(i))
                    + meshList[i].__repr__().partition("data=")[0]
                    for i in range(self.__field_distribution_count__)
                ],
            ]
        )
    ...
    def _checkSubNames(self):
        ...
        if len(set(self.__process_distribution_description__)) != len(
            self.__process_distribution_description__
        ):
        ...
        for i, nMode in enumerate(self.__mode_count__):
            for j in range(nMode):
                modeDescription.append(
                    self.__process_distribution_description__[i] + "_" + str(j)
                )
    ...
    def _checkCoefficients(self, coefficients):
        ...
        if (isinstance(coefficients, ot.Point), len(coefficients) == nModes):
            ...
            isinstance(coefficients, (ot.Sample, ot.SampleImplementation))
            and len(coefficients[0]) == nModes
        ):
            return True
        else:
    ...
    def getClassName(self):
        ...
        classNames = [
            self.__KLResultsAndDistributions__[i].__class__.__name__
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getCovarianceModel(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getCovarianceModel()
            if hasattr(self.__KLResultsAndDistributions__[i], "getCovarianceModel")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getEigenvalues(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getEigenvalues()
            if hasattr(self.__KLResultsAndDistributions__[i], "getEigenvalues")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getId(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getId()
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getImplementation(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getImplementation()
    ...
    def getMesh(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getMesh()
            if hasattr(self.__KLResultsAndDistributions__[i], "getMesh")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getModes(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getModes()
            if hasattr(self.__KLResultsAndDistributions__[i], "getModes")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getModesAsProcessSample(self):
        ...
        return [
            self.__KLResultsAndDistributions__[i].getModesAsProcessSample()
            if hasattr(self.__KLResultsAndDistributions__[i], "getModesAsProcessSample")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getProjectionMatrix(self):
        ...
        """
        return [
            self.__KLResultsAndDistributions__[i].getProjectionMatrix()
            if hasattr(self.__KLResultsAndDistributions__[i], "getProjectionMatrix")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getScaledModes(self):
        ...
        """
        return [
            self.__KLResultsAndDistributions__[i].getScaledModes()
            if hasattr(self.__KLResultsAndDistributions__[i], "getScaledModes")
            else None
            for i in range(self.__field_distribution_count__)
        ]
    ...
    def getScaledModesAsProcessSample(self):
        ...
        """
        return [
            self.__KLResultsAndDistributions__[i].getScaledModesAsProcessSample()
            if hasattr(self.__KLResultsAndDistributions__[i], "getScaledModes")
            else None
            for i in range(self.__field_distribution_count__)
        ]
        ...
        for i in range(self.__field_distribution_count__):
            if self.__isProcess__[i]:
                if not self.__liftWithMean__:
                    processes.append(
                        self.__KL_lifting__[i](
                            coefficients[:, jumpDim : jumpDim + self.__mode_count__[i]]
                        )
                    )
                else:
                    processSample = self.__KL_lifting__[i](
                        coefficients[:, jumpDim : jumpDim + self.__mode_count__[i]]
                    )
                ...
                if not self.__liftWithMean__:
                    ...
                    val_sample = self.__KL_lifting__[i](
                        coefficients[:, jumpDim : jumpDim + self.__mode_count__[i]]
                    )
    ...
    def liftAsField(self, coefficients):
        ...
        if valid:
            ...
            for i in range(self.__field_distribution_count__):
                if self.__isProcess__[i]:
                    field = self.__KLResultsAndDistributions__[i].liftAsField(
                        coefficients[jumpDim : jumpDim + self.__mode_count__[i]]
                    )
                    jumpDim += self.__mode_count__[i]
                    if not self.__liftWithMean__:
            ...
            raise Exception(
                "DimensionError : the vector of coefficient has the wrong shape"
            )
    ...
    def liftAsSample(self, coefficients):
        ...
        if valid:
            if self.__isAggregated__:
                if not self.__liftWithMean__:
                    sample = self.__KLResultsAndDistributions__[0].liftAsSample(
                        coefficients
                    )
                ...
                for i in range(self.__field_distribution_count__):
                    if self.__isProcess__[i]:
                        if not self.__liftWithMean__:
                            sample = self.__KLResultsAndDistributions__[i].liftAsSample(
                                coefficients[jumpDim : jumpDim + self.__mode_count__[i]]
                            )
    ...
    def project(self, args):
        ...
        for i in range(nArgs):
            addConstant2Iterable(
                args[i], -1 * self.__means__[i]
            )  # We then subtract the mean of each process to any entry, so we are again in the centered case
        ...
        if isAggreg:
            ...
            if isinstance(args[0], (ot.Field, ot.ProcessSample, ot.AggregatedFunction)):
                try:
                    fdi = args[0].getInputDimension()
                except:
                    fdi = args[0].getMesh().getDimension()
                try:
                    fdo = args[0].getOutputDimension()
                except:
                    fdo = args[0].getDimension()

                if fdi == inDim and fdo == outDim:
                    if nArgs > 1 and not isinstance(args[0], ot.ProcessSample):
                        sample = ot.Sample(
                            [self.__KL_projecting__[0](args[i]) for i in range(nArgs)]
                        )
                        sample.setDescription(self.__mode_description__)
                        return sample
                    elif isinstance(args[0], ot.Field):
                        projection = self.__KL_projecting__[0](args[0])
                        projDescription = list(
                            zip(self.__mode_description__, projection)
                        )
            ...
            if isinstance(args[0], (ot.Field, ot.Sample)):
                print(
                    "projection of a list of {} ".format(
                        ", ".join([args[i].__class__.__name__ for i in range(nArgs)])
                    )
                )
                assert (
                    nArgs == nProcess
                ), "Pass a list of same length then aggregation order"
                try:
                    projection = list()
                    for i in range(nProcess):
                        if (
                            isinstance(args[i], (ot.Sample, ot.Field))
                            and self.__isProcess__[i]
                        ):
                            projection.append(
                                list(
                                    self.__KLResultsAndDistributions__[i].project(
                                        args[i]
                                    )
                                )
                            )
                    ...
                    projectionFlat = [
                        item if not isinstance(item, (ot.Point)) else item[0]
                        for sublist in projection
                        for item in sublist
                    ]
                    output = ot.PointWithDescription(
                        list(zip(self.__mode_description__, projectionFlat))
                    )
                    return output
                ...
                try:
                    ...
                    for idx in range(nProcess):
                        if self.__isProcess__[idx]:
                            ...
                            distributionSample = ot.Sample(
                                [args[idx][i][0] for i in range(args[idx].getSize())]
                            )
                            projList.append(
                                self.__KL_projecting__[idx](distributionSample)
                            )
