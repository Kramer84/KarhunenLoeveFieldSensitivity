...
def function_with_save_wrapper(func):
    ...
    def new_function_wrapper(arg, file_name):
        if not (
            os.path.isfile(os.path.join(sample_path, file_name + ".csv"))
            ...
            vonMises = ot.Sample(
                np.array(
                    np.stack(
                        [
                            np.squeeze(np.asarray(output[0][i]))
            ...
            vonMises.exportToCSVFile(
                os.path.join(sample_path, file_name + "_VM" + ".csv"), ";"
            )
            maxDefl.exportToCSVFile(
                os.path.join(sample_path, file_name + "_MD" + ".csv"), ";"
            )
            ...
            sample_vonMises.setDescription(
                ot.Description.BuildDefault(len(vonMises.columns), "VM_")
            )
            sample_maxDefl = ot.Sample(maxDefl.values)
            sample_maxDefl.setDescription(
                ot.Description.BuildDefault(len(maxDefl.columns), "MD_")
            )
...
def getRandomNormalVector(AggregatedKLRes):
    nModes = (
        AggregatedKLRes.getSizeModes()
    )  # the number of elements in the input vector of our KL wrapped model
    ...
    if sample_size > limit:
        ...
        if rest > 0:
            outputs.append(
                func(
                    sample[
                        (n_reps + 1) * base_size : (n_reps + 1) * base_size + rest, :
                    ]
                )
            )
...
@timer
def getSobolExperiment(
    aggregated_karhunen_loeve_results, size, seed, secondOrder=False
):
    ot.RandomGenerator.SetSeed(seed)
    experiment = klfs.KarhunenLoeveSobolIndicesExperiment(
        aggregated_karhunen_loeve_results, size, secondOrder
    )
...
def isValidSobolIndicesExperiment(sample_like, size, second_order=False):
    ...
    print(
        "There are {} dimensions and {} sobol indices to calculate".format(
            dim, N_indice
        )
    )
    assert (
        np.where(Y_A == Y_B, True, False).any() == False
    )  # Here we check that there is no similarity at all  between A and B
    tot_lines = list()
    try:
...
class metamodeling_kriging:
    def __init__(self, inSample, outSample, **kwargs):
        ...
        self.__size_multistart__ = (
            kwargs["size_multistart"] if "size_multistart" in kwargs else 5
        )
        ...
        self.__optim_type__ = (
            kwargs["optim_type"] if "optim_type" in kwargs else "best_start"
        )
    ...
    def _get_results_metamodel(self):
        if isinstance(self.__kriging_theta__, (Sequence, Iterable, list)):
            self.__kriging_results__ = [
                _kt.getResult() for _kt in self.__kriging_theta__
            ]
            self.__kriging_metamodel__ = [
                _km.getMetaModel() for _km in self.__kriging_results__
            ]
            self.__kriging_model__ = [
                _km.getModel() for _km in self.__kriging_results__
            ]
...
class __validation_results__(object):
    ...
    def getResiduals(self):
        ...
        theCurve = ot.Curve(
            list(range(len(self.__residuals__))), self.__residuals__, "residuals"
        )
...
class _metamodel_parameter_routine:
    ...
    def _exec_sub_routine(self, AggregatedKLRes, FEM_model):
        ...
        doe_metasobol_experiment_N50000, _ = getSobolExperiment(
            AggregatedKLRes, 50000, SEED5
        )
        ...
        doe_sobol_experiment_N1000_VM, doe_sobol_experiment_N1000_MD = FEM_model(
            doe_sobol_experiment_N1000
        )
        doe_kriging_valid_VM, doe_kriging_valid_MD = FEM_model(doe_kriging_valid)
        R2_LHS25, kriging_model_LHS25 = self.metamodel_kriging_validation(
            doe_kriging_LHS25,
            doe_kriging_LHS25_MD,
            doe_kriging_valid,
            doe_kriging_valid_MD,
        )
        R2_LHS50, kriging_model_LHS50 = self.metamodel_kriging_validation(
            doe_kriging_LHS50,
            doe_kriging_LHS50_MD,
            doe_kriging_valid,
            doe_kriging_valid_MD,
        )
        R2_LHS100, kriging_model_LHS100 = self.metamodel_kriging_validation(
            doe_kriging_LHS100,
            doe_kriging_LHS100_MD,
            doe_kriging_valid,
            doe_kriging_valid_MD,
        )
        # Now we have to calculate the sobol indices
        result_point_DOE1000 = self.getSensitivityAnalysisResults(
            doe_sobol_experiment_N1000,
            doe_sobol_experiment_N1000_MD,
            1000,
            N_KL_Young,
            N_KL_Diam,
        )
        ...
        print("LHS25")
        result_point_DOE50000_LHS25 = self.getSensitivityAnalysisResultsMetamodel(
            kriging_model_LHS25,
            doe_metasobol_experiment_N50000,
            50000,
            25,
            N_KL_Young,
            N_KL_Diam,
        ...
        print("LHS50")
        result_point_DOE50000_LHS50 = self.getSensitivityAnalysisResultsMetamodel(
            kriging_model_LHS50,
            doe_metasobol_experiment_N50000,
            50000,
            50,
        ...
        print("LHS100")
        result_point_DOE50000_LHS100 = self.getSensitivityAnalysisResultsMetamodel(
            kriging_model_LHS100,
            doe_metasobol_experiment_N50000,
            50000,
            100,
            N_KL_Young,
        ...
        ResultsSample = ot.Sample(
            [
                result_point_DOE1000,
                result_point_DOE50000_LHS25,
                result_point_DOE50000_LHS50,
                result_point_DOE50000_LHS100,
            ]
        )
    ...
    def exporteSampleAsCsv(self, listSamples, listNames):
        ...
        for i, sample in enumerate(listSamples):
            sample.exportToCSVFile(
                os.path.join("./segfault_analysis/debug_storage", listNames[i])
            )

    def getSensitivityAnalysisResults(
        self, sample_in, sample_out, size, N_KL_Young, N_KL_Diam
    ):
        ...
        result_point = ot.PointWithDescription(
            [
                ("meta", 0.0),
                ("N_LHS", -1.0),
                ("size", size),
                ("kl_dimension", dim),
                ("N_KL_Young", N_KL_Young),
                ("N_KL_Diam", N_KL_Diam),
                ("R2", -1),
                ("SI_E", round(FO_indices[0][0], 5)),
                ("SI_E_lb", round(lb[0], 5)),
                ("SI_E_ub", round(ub[0], 5)),
                ("SI_D", round(FO_indices[1][0], 5)),
                ("SI_D_lb", round(lb[1], 5)),
                ("SI_D_ub", round(ub[1], 5)),
                ("SI_FP", round(FO_indices[2][0], 5)),
                ("SI_FP_lb", round(lb[2], 5)),
                ("SI_FP_ub", round(ub[2], 5)),
                ("SI_FN", round(FO_indices[3][0], 5)),
                ("SI_FN_lb", round(lb[3], 5)),
                ("SI_FN_ub", round(ub[3], 5)),
            ]
        )
        print("------------- RESULTS ------------")
        print("------------ REAL MODEL ----------")
    ...
    def getSensitivityAnalysisResultsMetamodel(
        self, metamodel, doe_sobol, size, N_LHS, N_KL_Young, N_KL_Diam, R2
    ):
        dim = doe_sobol.getDimension()
        try:
            meta_response = sequentialFunctionEvaulation(
                metamodel.__kriging_metamodel__, doe_sobol
            )
        ...
        result_point = ot.PointWithDescription(
            [
                ...
                ("SI_D_lb", lb[1]),
                ("SI_D_ub", ub[1]),
                ("SI_FP", FO_indices[2][0]),
                ("SI_FP_lb", lb[2]),
                ("SI_FP_ub", ub[2]),
                ("SI_FN", FO_indices[3][0]),
                ("SI_FN_lb", lb[3]),
                ("SI_FN_ub", ub[3]),
            ]
        )
    ...
    def metamodel_kriging_validation(
        self, doe_in, doe_out, doe_validation_in, doe_validation_out
    ):
        kriging_model = metamodeling_kriging(
            doe_in, doe_out, optim_type="multi_start", size_multistart=10
        )
        kriging_model.run()
        kriging_model.getMetaModelValidation(doe_validation_in, doe_validation_out)
        R2 = kriging_model.validation_results.getR2()[
            0
        ]  # for now works only with 1D outputs
    ...
    def getBasePathAndFileName(self):
        file_name = (
            "EXP_"
            + "VE"
            + str(int(self.variance_young))
            + "_"
            + "SE"
            + str(int(self.scale_young))
            + "_"
            + "VD"
            + str(round(self.variance_diam, 4))
            + "_"
            + "SD"
            + str(int(self.scale_diam))
            + "_"
            + "VFP"
            + str(round(self.var_f_pos, 3))
            + "_"
            + "VFN"
            + str(round(self.var_f_norm, 3))
            + "_"
            + "NU"
            + str(round(self.nu, 3))
            + ".csv"
    ...
    def getKLDecompositionYoungDiam(self):
        kl_results_E = get_process_kl_decomposition(
            mean=self.mean_young,
            amplitude=self.variance_young,
            scale=self.scale_young,
            nu=self.nu,
            mesh=self.fem_vertices,
            dimension=dim_field,
            name="E_",
        ...
        kl_results_D = get_process_kl_decomposition(
            mean=self.mean_diam,
            amplitude=self.variance_diam,
            scale=self.scale_diam,
            nu=self.nu,
            mesh=self.fem_vertices,
            dimension=dim_field,
            name="D_",
    ...
    def getAggregatedKLResults(self, kl_results_E, kl_results_D):
        ...
        AggregatedKLRes.setMean(
            0, self.mean_young
        )  # At other indices the means are initialized from the distributions
...
class globalMetaParameterAnalysis:
    def __init__(
        ...
        var_scale_Young = ot.Uniform(0.2 * scale_beam, 1.0 * scale_beam)
        var_ampl_Diam = ot.Uniform(0.001 * mean_diam, 0.1 * mean_diam)
        var_scale_Diam = ot.Uniform(0.2 * scale_beam, 1.0 * scale_beam)
        ...
        ComposedMetaParamDistribution = ot.ComposedDistribution(
            [
                var_ampl_Young,
                var_scale_Young,
                var_ampl_Diam,
        ...
        RandomNormalVector = ot.ComposedDistribution(
            [ot.Normal()] * 7
        )  # Vector used for the LHS generation
        normalizedLHS = optimizedLHS(RandomNormalVector, 100, SEED)
        inverseIsoProbaTransform = (
            ComposedMetaParamDistribution.getInverseIsoProbabilisticTransformation()
        )
