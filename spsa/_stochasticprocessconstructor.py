...
class StochasticProcessConstructor(openturns.Process):
    ...
    def setGrid(self, grid_shape : list):
        ...
        assert(len(grid_shape)  == self.dimension \
            and type(grid_shape[0]) is list), 'check self.printHelp method' 
        ...
        self.extent = numpy.ravel(numpy.asarray(
                                  list(zip(low_bounds,high_bounds)))).tolist()
    ...
    def setCovarianceModel(self, covarianceModelDict : dict = None, 
        ...
        if covarianceModel is not None :
            assert type(covarianceModel) in \
                    list(zip(*OTCovarModels.values()))[0], \
                                str(covarianceModel) \
                                    + " is not implemented in openturns yet" 
            ...
            assert(covarianceModelDict['Model'] in OTCovarModels), \
              "only use the same model names than those existing in openTURNS"
        ...
        try : 
            ...
            print("Can't set gaussian process, some error occured.\
                Verifiy your parameters")
    ...
    def setTrend(self, arguments : list, funcOrConst = 0):
        ...
        assert self.mesh is not None,\
                                     'first set grid with self.setGrid method'
        ...
        if self.verbosity > 0 :
            print(
                'trend function args: ',self._trendArgs,
                ' trend function: ', self._trendFunc,'\n'
                )
        if self.verbosity > 1 :
            print(
                'Please be aware that the number of elements in the argument\
                list has to be the same as the dimension of the process: ', 
                self.dimension
                )
...
##############################################################################
    ...
    def getKarhunenLoeveDecomposition(self, method = 'P1Algorithm',  
        ...
        try :
            assert(self.GaussianProcess is not None \
                            and self.mesh is not None), "First create process"
        except AssertionError :
    ...
    def liftDistributionToField(self, randomVector):
        ...
        try : 
            ...
            self.getKarhunenLoeveDecomposition()
        if type(randomVector[0]) == float : # only one realisation
            field = self.KarhunenLoeveResult.liftAsField(randomVector)
            return field 
        elif type(randomVector[0]) == list: # if a collection of realisations
...
##############################################################################
    def getProcessSampleFromNumpyArray(self, ndarray:numpy.ndarray=None)\
