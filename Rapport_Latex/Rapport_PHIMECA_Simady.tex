...
En deux dimensions, un champ stochastique peut être vu comme un maillage uniforme, ou une valeur est associée à chaque noeud, et ou la valeur à chaque noeud est colérée à la distance et valeur des noeuds proches. La première idée aurait donc été d'appliquer directement une déformation génerée par un champ de la même taille que le maillage d'une ailette aux noeuds constituant l'ailette, et donc de simuler
une déformation stochastique par ailette, mais en écartant du coup toute corrélation inter-ailettes.
La déformation se serait fait par translation de chaque noeud suivant l'axe normal au plan de l'ailette.
Cette méthode a vite été écartée, puisque la génération de champs sur des maillages sous forme de long rectangle étiré était bien moins précise et controlables, allant jusqu'à des réalisation quasi constantes. De plus le nombre de champs genéré par plaque serait très important, alors qu'il serait plus simple de ne genérér qu'un seul champ par plaque et d'user de déformation paramétriques. \\

Cette seconde approche est inspirée d'un travail par CITEGOKA, qui traite de la modélisation des déformations réelles de pièces mécaniques. Il découle de son travail, que la déformation d'une pièce physique peut être représenté par une sommes de modes de déformation primaires, et qui similairement à une décompositin de fourrier, peut finalement représenter toute déformation de pièce. \\

En éxprimant la déformation de chaque arc de neuds colinéaire à la direction normale aux deux flux d'air, comme une fonction mathématique représentant cette déformation basique, par exemple un polynôme du second degré possédant comme racines les deux positions extrêmes de l'arc, et en rendant cette fonction pilotée par un unique paramètre, l'on peut appliquer facilement une déformation à chaque arc de la plaque crénelée. Cette déformaiton, en plus d'être physique, possède une corrélation intra-ailettes, et ne nécessite que la génération d'un seul champ par plaque crénelée. De plus on peut facielement superposer les déformations par sommation.

Un exemple de déformation qu'il est possible d'obtenir avec cette méthode (un seul mode de déformation): 
