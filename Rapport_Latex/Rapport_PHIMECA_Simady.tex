...
Pour générer les samples de manière plus efficace, et être sur de couvrir l'ensemble du domaine de variance des lois d'entrée, 
%FAUX Comme l'utilisation des codes dans openTURNS est fortement imbriquée (L'on ne peut faire l'analyse de Sobol' que lorsque les données arrivent comme cela est prévu par openTURNS), il nous faut générer les Samples différement et revoir la manière dont on récupère les indices de Sobol'. Pour la génération des samples, on va utiliser la méthode du LHSMDU (Latin Hypercube Sampling with Multidimensional Uniformity) par \cite{Deutsch2012Mar} et l'implémentation Python de \cite{Moza2019Feb}.  
